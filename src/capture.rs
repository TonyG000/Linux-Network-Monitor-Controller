...
impl CaptureEngine {
    // Open and validate a capture handle on iface
    ...
    // Parses each frame and calls on_packet with the result
    // Returns only on an unrecoverable pcap error; Ctrl+C terminates the process via the default SIGINT handler.
    ...
    // list every network interface available on this machine.
